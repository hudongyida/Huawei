...
- 值越小优先级越高
- 每一个路由协议都有想对应的路由优先级| 路由来源                                         | 路由类型     | 默认优先级 |
  | ------------------------------------------------ | ------------ | ---------- |
  | 直连                                             | 直连         | 0          |
  | 静态                                             | 静态         | 60         |
  | 动态                                             | OSPF内部路由 | 10         |
...
- 无AC FAT【胖AP】架构：由AP直接和园区网关连接，具有独立实现WLAN的能力，不受AC管控
- AC+FIT【瘦AP】架构：由于瘦AP无法单独实现WLAN功能，绝大多数的设置需要AC配置，所以受AP需要先和AC建立CAPWAP隧道链接，带连上AP后，再由AC设置是直连组网还是旁挂组网
- 敏捷分布式AP架构：在AC+FIT的基础上做的改进，在瘦AP的范围上做了更加细致的划分，将瘦AP拆分成中心AP和敏分AP，中心AP负责接受AC的配置，敏分AP仅是负责将无线接入点传个中心AP

CAPWAP协议：

1. AP会周期性的发送CAPWAP关播帧，以便于让AC发现
2. AC检测到AP后会，将AP添加到管控列表后，AC边会和AP建立CAPWAP隧道，以专门用来传输AC的配置信息

有线 AP-AC链接/组网
组网：

- 二层组网：AC AP在二层之间组网，只能设置一个网络，共享同一个广播域
- 三层组网：AC AP在三层之间组网，一个AC可以配置多个网络，通常在大型企业网络中采用
  链接：
- 直连【数据隧道】：AP收集到的所有数据都必须进过AC转发，包括业务数据
- 旁挂【控制隧道】：AC仅和AP采用CAPWAP隧道链接来传输配置，而AP收集到的数据则由AP直接方式到网段内，不再经过AC。  *隧道内只有AC下发的控制信息*

无线侧组网概念：
BSS: 基本服务集群【及一个AP所覆盖的范围
BSSID： 基本服务集标识符【全球唯一
SSID: 服务标识符【便于用户识别不同的网络
![无线侧组网概念](./img/无线侧组网概念.png)

VAP： 虚拟AP，一个物理AP可以划分成N个虚拟的AP
ESS： 扩展服务集，采用多个相同的SSID组成的BSS，以此来实现无线漫游

WLAN工作原理：

1. AP上线，获取IP地址【静态获取，DHCP获取】
2. AP发送广播报文，AP发现AC并且建立CAPWAP隧道【AP发生Discovery Request报文，建立CAPWAP需要根据AC配置查明，是数据隧道或还是控制隧道】
3. AP接入控制，并且维持CAPWAP隧道。【AP发送Join Request报文,维持隧道时AC AP直接会交换keepalive*数据隧道*  和控制隧道*Echo* 两种报文来确保隧道的连通状态】
4. AC下发配置文件【各种模板】
5. 用户接入【扫描，链路验证，关联，接入，DHCP，用户认证】
6. 用户上线

模板配置：

- 域管理模板：提供AP的国家码【每个国家规定可以使用的无线频段不一样】，调整信道集合和带宽配置等
- 射频模板：优化射频参数
- VAP模板: 在VAP模板下配置各各种参数，和引用SSID模板，安全，认证模板等
  1. 创建SSID模板
  2. 创建安全模板，配置无线的安全信息和对用户报文进行加密，保护WLAN用户安全
  3. 配置转发报文
  4. 配置VLAN业务
- 其他模板：AP系统模板，定位模板等
...
1. 扫描：接入设备可以主动扫描周围的无线网络，也可以被动扫描【侦听AP发出的Beacon帧】
2. 认证：为WLAN提供数据安全，并且提供了WEP WPA WPA2等安全机制，并且从链路认证和接入认证两个安全的链接

   - 链路认证： 无线设备需要预先和AP配置相同的秘钥，在链接时无线接入设备会先发生认证请求。随后AP会发出一个随机代码，无线接入设备会用预习配置好的秘钥加密，将加密后的内容发回AP，AP收到后会用秘钥解密，若解密后的内容和之前AP发出服随机代码一直，则认定设备合法，链路认证完成。否则则是认证失败 **链路认证不是WLAN接入的必要步骤，大部分的链路认证都是开发式的，及不进行链路认证，如WPA WPA2等**

   **注意：在链路建立之后会进入关联阶段，用于协商速率信道等，待协商完成后再根据配置觉得是否进入接入认证阶段**

   - 接入认证：对用户进行分区，并且对用户访问网络的时候进行必要的权限设置，相较链路认证安全性更好，功能更加强大
3. 地址分配/完成接入： 根据网络内配置的DHCP服务器自动获取网络地址
...
1. 建立链路并且进行LCP协商，建立LCP链路【LCP协商失败则回滚到0.链路静止
2. LCP链路建立链接成功后进入身份鉴别【鉴别失败则进入 **a.链路终止** 阶段，并且关闭LCP链路，回滚至0.链路静止
3. 身份鉴别成功/无需鉴别 进入网络配置商议 NCP协商阶段【NCP配置协商失败者进入 **b.链路故障或关闭请求** 阶段，随后进入**a.链路终止** 阶段
4. NCP协商好后PPP链路打开【如需关闭链路这进入**b.链路故障或关闭请求** 阶段，后转入**a.链路终止** 阶段，关闭LCP链路后回归0. 链路静止状态

【***有关PPP的更详细内容请将参见 计算机网络第八版 笔记 数据链路层***，包括PPPoE】
